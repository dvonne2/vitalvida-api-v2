...
class SettingsController extends Controller
{
    ...
    public function update(Request $request): JsonResponse
    {
        ...
        try {
            ...
            // Update configuration files (in a real app, you'd use a settings table)
    ...
    public function backup(): JsonResponse
    {
        try {
            ...
            return response()->json([
                ...
                'download_url' => url('storage/backups/settings/' . $filename)
            ]);
    ...
    public function restore(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $filePath = storage_path('app/backups/settings/' . $backupFile);
            ...
            // Apply the settings
            $this->applySettings($backupData['settings']);
            ...
            SystemLog::create([
                'level' => 'info',
                'message' => 'System settings restored from backup',
                'context' => [
                    'restored_by' => auth()->user()->username,
                    'backup_file' => $backupFile,
                    'backup_exported_at' => $backupData['exported_at'] ?? 'unknown'
                ],
                'source' => 'settings_controller'
            ]);
    ...
    {
        try {
            $backupDir = storage_path('app/backups/settings');
            $backups = [];
            ...
            if (file_exists($backupDir)) {
                ...
                foreach ($files as $file) {
                    ...
                    $backups[] = [
                        ...
                        'exported_by' => $backupData['exported_by'] ?? 'unknown',
                        'download_url' => url('storage/backups/settings/' . $filename)
                    ];
                }
            }
